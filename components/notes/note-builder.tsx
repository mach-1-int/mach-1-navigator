...
import { AiRecorder } from "./ai-recorder"
import type { TemplateFieldContext } from "@/lib/gemini-scribe"
import type { NoteTemplate, TemplateField, UserRole, ZCode, TimeLog } from "@/lib/types"
...
export function NoteBuilder({
  ...
  // Update manual narrative when switching to manual mode
  useEffect(() => {
    if (isManualOverride && manualNarrative === "") {
      setManualNarrative(autoGeneratedNarrative)
    }
  }, [isManualOverride, autoGeneratedNarrative, manualNarrative])
  ...
  const handleAiAutoFill = useCallback((data: Record<string, unknown>, fieldsMatched: number, isMock: boolean) => {
    console.log("🎨 handleAiAutoFill called with:", { data, fieldsMatched, isMock });
    ...
    setResponses((prev) => {
      ...
      console.log("🎨 New responses after merge:", newResponses);
      return newResponses;
    })
    ...
    toast({
      ...
      description: `${fieldsMatched} field${fieldsMatched !== 1 ? "s" : ""} populated from transcript${isMock ? " (Demo Mode)" : ""}`,
    })
  ...
  const handleSubmit = () => {
    ...
    try {
      ...
      // Prepare responses with billing data
      const finalResponses = isManualOverride
        ? { ...responses, _manualNarrative: manualNarrative }
        : responses

      // Add billing metadata to responses
      finalResponses._modality = modality
      finalResponses._barrierAddressed = barrierAddressed
      finalResponses._goalAlignment = goalAlignment
            ...
            <div className="flex flex-1 min-h-0">
              ...
              <div className="flex-1 overflow-y-auto border-r">
                <div className="p-6 space-y-6">
                  ...
                  {aiAutoFillCount !== null && (
                    <div className={cn(
                      "flex items-center gap-2 p-3 rounded-lg animate-in fade-in slide-in-from-top-2 duration-300",
                      aiIsMock

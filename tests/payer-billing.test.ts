/**
 * Vitest port of scripts/verify-payer-billing.ts (Payer-Agnostic Billing Engine).
 *
 * NOTE: This suite uses RAW (unrebased) seed data with billingPeriod "2026-01",
...
describe("Payer-Agnostic Billing Engine", () => {
  it("Step 1–2: Default payer Arizona Medicaid (H-Codes) and rate card", () => {
  ...
  it("Step 2: Rule of Eights – 45 min = 3 units, $55.50", () => {
  ...
  it("Step 3: Medicare PIN – rate card and thresholds", () => {
  ...
  it("Step 6: CSV Billing_Model column (MEDICAID BH, MEDICARE PIN)", () => {
    const medicaid = getPayerConfig("medicaid-bh")
  ...
  it("Step 7 & 10: Initial state default payer and reset", () => {
  ...
  it("Claims regeneration by payer", () => {
    const medicaid = getPayerConfig("medicaid-bh")
    const pin = getPayerConfig("medicare-pin")
    const claimsM = generateMonthlyClaims(initialNavigators, initialPatients, initialTimeLogs, medicaid, initialIntakeRecords)
    const claimsG = generateMonthlyClaims(initialNavigators, initialPatients, initialTimeLogs, pin, initialIntakeRecords)
  ...
  it("Revenue metrics use active payer config", () => {
    const medicaid = getPayerConfig("medicaid-bh")
    const pin = getPayerConfig("medicare-pin")
    const claimsM = generateMonthlyClaims(initialNavigators, initialPatients, initialTimeLogs, medicaid, initialIntakeRecords)
    const claimsG = generateMonthlyClaims(initialNavigators, initialPatients, initialTimeLogs, pin, initialIntakeRecords)
  ...
  it("QA Scenario 1: Guardrail (45 min → Needs Attention, not Ready)", () => {
    const pin = getPayerConfig("medicare-pin")
  ...
  it("QA Scenario 2: Logic Engine (75 min → G0023; 105 min → G0023 + G0024)", () => {
    const pin = getPayerConfig("medicare-pin")
  ...
  it("QA Scenario 3: Validation (Missing Member ID)", () => {
    const pin = getPayerConfig("medicare-pin")
  ...
  it("QA Scenario 4: Bridge (CSV columns and Date_Of_Service)", () => {
    const pin = getPayerConfig("medicare-pin")
    const claims = generateMonthlyClaims(initialNavigators, initialPatients, initialTimeLogs, pin, initialIntakeRecords)
  ...
  it("Consent guardrail: pt-validation-test flags missing consent", () => {
    const medicaid = getPayerConfig("medicaid-bh")
  ...
  it("12-month rule: stale initiating visit flags re-initiation", () => {
    const medicaid = getPayerConfig("medicaid-bh")
  ...
  it("CHI Z-code rule: CHI claim without Z-codes flags SDOH error", () => {
    const medicaid = getPayerConfig("medicaid-bh")
  ...
  it("Intake Z-code merge: CHI patients pass via seeded intake barriers", () => {
    const medicaid = getPayerConfig("medicaid-bh")
    const claims = generateMonthlyClaims(initialNavigators, initialPatients, initialTimeLogs, medicaid, initialIntakeRecords)
  ...
  it("Payer identity: memberId and payerId on claims", () => {
    const medicaid = getPayerConfig("medicaid-bh")
    const claims = generateMonthlyClaims(initialNavigators, initialPatients, initialTimeLogs, medicaid, initialIntakeRecords)
  ...
  it("Seed integrity: patient payerIds and time log navigatorIds resolve", () => {
